...
type IService interface {
	...
	Query(ctx context.Context, query *selection_condition.SelectionCondition) ([]Client, error)
}
...
func (s service) NewEntity() *Client {
	return New()
}

// Get returns the entity with the specified ID.
func (s service) Get(ctx context.Context, id uint) (*Client, error) {
...
func (s service) Query(ctx context.Context, query *selection_condition.SelectionCondition) ([]Client, error) {
	items, err := s.repository.Query(ctx, query)
	if err != nil {
		return nil, errors.Wrapf(err, "Can not find a list of Client by query: %v", query)
	}
	return items, nil
}
